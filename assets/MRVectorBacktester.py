...
class MRVectorBacktester(MomVectorBacktester):
    ...
    def run_strategy(self, SMA, threshold):
        ...
        thrs = max([abs(data["distance"].min()), abs(data["distance"].max())]) * threshold
        # sell signals
        data['position'] = np.where(data['distance'] > thrs,
                                    -1, np.nan)
        # buy signals
        data['position'] = np.where(data['distance'] < -thrs,
                                    1, data['position'])
    ...
    def plotx_results(self):
        ...
        '''
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=self.results["Date"], y=self.results["distance"],
                    mode='lines',
                    name='distance'))
        fig.show()
