...
from assets import LRVectorBacktester as LR
...
def plotreturns(asset, data, tc, lag):
    title = 'Gross performance compared to the linear regression strategy<br><sup> %s | TC = %.1f | Lag = %.f' % (asset, tc, lag)
    fig = go.Figure()
...
def plotfsn(data, res, asset, sdate_lrn, edate_lrn, sdate, edate):
    ...
    fig.update_layout(title="Price chart & learning/backtesting ranges", legend=dict(
        orientation="h",
        yanchor="bottom",
        y=1.02,
        xanchor="right",
...
input_groups = html.Div(
    [
        dbc.InputGroup(
            [dbc.InputGroupText("Category", id="category_header_lr"), dbc.Select(options=[
                {"label": "Stocks", "value": "Stocks"},
                {"label": "Crypto", "value": "Crypto"}],
                id='category-dropdown_lr',
                value="Stocks")],
            className="mb-3",
        ),
        dbc.InputGroup(
            [dbc.InputGroupText("Asset", id="asset_header_lr"), dbc.Select(options=std,
                                                                           value="TSLA",
                                                                           id='assets-dropdown_lr')],
            className="mb-3",
        ),
        dbc.InputGroup(
            [
                dbc.InputGroupText("Training", id="datelrn_header_lr"),
                dcc.DatePickerRange(
                    id='date-picker-range_lrn_lr',
        ...
        dbc.InputGroup(
            [
                dbc.InputGroupText("Backtesting", id="date_header_lr"),
                dcc.DatePickerRange(
                    id='date-picker-range_lr',
                    display_format='DD/MM/YYYY',
                    start_date_placeholder_text="Start Date",
                    end_date_placeholder_text="End Date",
                    min_date_allowed=date(2020, 1, 1),
        ...
        dbc.InputGroup(
            [
                dbc.InputGroupText("Lag", id="lag_header_lr"),
                dbc.Input(id="Laginput_lr", type="number", value=3, min=1, max=15, step=1, style={})
            ],
            className="mb-3",
        ),
        dbc.InputGroup(
            [
                dbc.InputGroupText("Transaction cost", id="tc_header_lr"),
                dbc.Input(id="TCinput_lr", type="number", value=0, min=0, max=5, step=0.1, style={}),
                dbc.InputGroupText("%"),
            ],
            className="mb-3",
        ),
        dbc.InputGroup(
            [dbc.InputGroupText("Start capital", id="sc_header_lr"),
             dbc.Input(id="SCinput_lr", type="number", value=10000, min=1, max=9999999999, step=1, style={}),
             dbc.InputGroupText("$")
             ],
            className="mb-3"
        )
...
sidebar_lr = html.Div(
    [
        html.H2("Linear Regression", className="display-4"),
        html.Hr(),
        html.P(
            "Strategy parameters", className="lead"
        ),
        input_groups,
    ], style=sidebar_hide, id="sidebar_lr"
)
dashboard = html.Div(children=
                     [dbc.Row([dbc.Col(dcc.Graph(figure=fs, id="strat_lr"), width=8),
                               dbc.Col([dcc.Graph(figure=fighist, id="shist_lr")], width=4)],
                              style={'padding-left': 10, 'padding-right': 10, 'flex': 1}),
                      dbc.Row([dbc.Col([dcc.Graph(figure=fig, id="splot_lr")], width=8),
                               dbc.Col([dcc.Graph(figure=rfig, id="res_lr")], width=4)]),
                      dbc.Modal(
                          [
                              ...
                              dbc.ModalBody(children=[
                                  ...
                                  html.Hr(),
                                  html.P(
                                      "Linear regression is used to derive a financial instrument's direction of future price movement."),
                                  html.Br(),
                                  ...
                                  html.P("The upper part of the graph shows asset price, training and backtesting areas. The lower part of the graph shows the accuracy with which the model predicts the direction of price movement."),
                                  html.Li("Gross performance compared to the linear regression strategy"),
                                  html.P(
                      ...
                      dbc.Popover(
                          ...
                          target="lag_header_lr",
                          body=True,
                          trigger="hover",
                      )
                      ], id="page-content_lr", style=content_full)

# Design the app layout
layout = html.Div(
    [dcc.Store(id='side_click_lr'), navbar, dbc.Row([dbc.Col(sidebar_lr), dbc.Col(dashboard)])]), html.Div(dcc.Loading(
    id="loading",
    children=[html.Div([html.Div(id="loading-output_lr")])],
    type="cube",
    color="#7F7F7F"),
...
@dash.callback(
    dash.dependencies.Output('assets-dropdown_lr', 'options'),
    dash.dependencies.Output('assets-dropdown_lr', 'value'),
    [dash.dependencies.Input('category-dropdown_lr', 'value')])
...
@dash.callback(
    dash.dependencies.Output("modal_lr", "is_open"),
    [dash.dependencies.Input("btn_about", "n_clicks"), dash.dependencies.Input("close", "n_clicks")],
    [dash.dependencies.State("modal_lr", "is_open")],
)
...
@dash.callback(
    dash.dependencies.Output("sidebar_lr", "style"),
    dash.dependencies.Output("page-content_lr", "style"),
    dash.dependencies.Output("side_click_lr", "data"),
    dash.dependencies.Output("btn_sidebar_lr", "children"),

    [dash.dependencies.Input("btn_sidebar_lr", "n_clicks")],
    [
        dash.dependencies.State("side_click_lr", "data"),
    ]
)
...
@dash.callback(
    dash.dependencies.Output('splot_lr', 'figure'),
    dash.dependencies.Output('shist_lr', 'figure'),
    dash.dependencies.Output('strat_lr', 'figure'),
    dash.dependencies.Output('res_lr', 'figure'),
    dash.dependencies.Output('date-picker-range_lr', 'min_date_allowed'),
    dash.dependencies.Output('date-picker-range_lr', 'max_date_allowed'),
    dash.dependencies.Output('date-picker-range_lrn_lr', 'min_date_allowed'),
    dash.dependencies.Output('date-picker-range_lrn_lr', 'max_date_allowed'),
    dash.dependencies.Output("loading-output_lr", "children"),
    [dash.dependencies.Input('assets-dropdown_lr', 'value'),
     dash.dependencies.Input('TCinput_lr', 'value'),
     dash.dependencies.Input('SCinput_lr', 'value'),
     dash.dependencies.Input('date-picker-range_lr', 'start_date'),
     dash.dependencies.Input('date-picker-range_lr', 'end_date'),
     dash.dependencies.Input('date-picker-range_lrn_lr', 'start_date'),
     dash.dependencies.Input('date-picker-range_lrn_lr', 'end_date'),
     dash.dependencies.Input('Laginput_lr', 'value')
     ])
def upd_chart(asset, tc, sc, sdate, edate, sdate_lrn, edate_lrn, lag):
    lrbt = LR.LRVectorBacktester(asset, min(sdate, edate, edate_lrn, sdate_lrn),
                                 max(sdate, edate, edate_lrn, sdate_lrn), sc, tc / 100)

    raw = lrbt.get_raw().reset_index()
    mind = raw["Date"].min()
    maxd = raw["Date"].max()
    ...
    lrbt.run_strategy(sdate_lrn, edate_lrn, sdate, edate, lags=lag)
    res = lrbt.get_results()
    resfs = res.copy()
    ...
    return fig, fighist, fs, rfig, mind, maxd, mind, maxd, load
